...
def createprofile(request):
    data = {'update_user_form': UpdateUserForm(instance=request.user),
            'accomodations_preference_count': range(ACCOMODATION_PREFERENCE_LIMIT), 'form': None}
    log.info("create profile form", extra=request.log_extra)
    try:
        user_profile = request.user.userprofile
        data['note'] = _("You can update your profile below")
        data['form'] = StuProfileForm(instance=user_profile)
        if not UserProfileOPS.is_instructor(user_profile):
            log.debug("egitmen olmayan kullanici icin isleme devam ediliyor", extra=request.log_extra)
            data['accomodations'] = Accommodation.objects.filter(
                    usertype__in=['stu', 'hepsi'], gender__in=[user_profile.gender, 'H'], site=request.site).order_by(
                    'name')
            data['accomodation_records'] = UserAccomodationPref.objects.filter(user=user_profile).order_by(
                    'preference_order')
    except:
        data['note'] = _("If you want to continue please complete your profile.")
        data['form'] = StuProfileForm()
        data['accomodations'] = Accommodation.objects.filter(usertype__in=['stu', 'hepsi'], gender__in=['K', 'E', 'H'],
                                                             site=request.site).order_by('name')
                        ...
                        for pref in range(0, len(data['accomodations'])):
                            if 'tercih' + str(pref + 1) in request.POST.keys():
                                try:
                                    ...
                                    uaccpref.save()
                                    data[
                                        'note'] = ("Profiliniz başarılı bir şekilde kaydedildi."
                                                   " Kurs tercihleri adımından devam edebilirsiniz")
                                except Exception as e:
                                    log.error(e.message, extra=request.log_extra)
                                    data['note'] = ("Profiliniz kaydedildi ancak konaklama tercihleriniz kaydedilemedi."
                                                    " Sistem yöneticisi ile görüşün!")
                    else:
                        data['note'] = ("Profiliniz başarılı bir şekilde kaydedildi."
                                        " Kurs tercihleri adımından devam edebilirsiniz")
                except Exception as e:
                    log.error(e.message, extra=request.log_extra)
                    data['note'] = ("Profiliniz kaydedilirken hata oluştu lütfen sayfayı"
                                    " yeniden yükleyip tekrar deneyin")
            else:
