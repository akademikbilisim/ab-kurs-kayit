...
from abkayit.adaptor import send_email, define_crontab, deleteoldjobs
...
def definenotapprovedtrainesscronjob_signal(instance, created, **kwargs):
    ...
    log.info("not_approved_trainess_eventstardate defined for date %s" % beforeeventstartdate, extra={'clientip': '', 'user': ''})
    allapprovedates = ApprovalDate.objects.filter(site=instance, for_instructor=True).order_by('-end_date')
    if allapprovedates:
        ...
        log.info("not_approved_trainess_after_approval_period_ends defined "
                 "for date %s" % (allapprovedates[0].end_date + timedelta(days=1)), extra={'clientip': '', 'user': ''})
