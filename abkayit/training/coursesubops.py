...
class CourseSubOps():
    def create_trainer(self, postrequest, numofinst):
        insts = []
        uprof = UserProfileOPS()
        for i in xrange(numofinst):
            n_str = str(i) + '-'
            upass = uprof.generate_new_pass()  # uretilen parola olusturulan kullaniciya gonderilecek.
    ...
    def create_course(self, request, insts):
        try:
