...
def submitandregister(request):
    ...
    try:
        ...
        logger.info("%s kullanici profili bulunamadi" % (request.user), extra=d)
...
def list_courses(request):
    ...
    data = dict()
    try:
        courses = Course.objects.filter(site=Site.objects.get(is_active=True))
...
def apply_to_course(request):
    ...
    if userprofile.userpassedtest:
        ...
        additipnal_pref_start_for_trainess = ApprovalDate.objects.get(site=data['site'], preference_order=1,
        ...
        if now < data['site'].application_start_date:
            ...
            data['closed'] = "1"
            if len(TrainessCourseRecord.objects.filter(trainess_approved=True).filter(preference_order__gte=0).filter(
                    trainess=userprofile)) == 0:
                try:
                    ...
                    if now_for_pref > additional1_pref_for_trainess.start_date and now_for_pref < additional1_pref_for_trainess.end_date:
                        ...
                        logger.debug("ek tercih aktif", extra=d)
...
def control_panel(request):
    ...
    try:
        ...
        if not uprofile:
            ...
            if courses:
                logger.info("egitmenin " + str(len(courses)) + " tane kursu var", extra=d)
                trainess = {}
                ...
                third_pref_approve_end = ApprovalDate.objects.get(site=data['site'], preference_order=3,
                                                                  for_instructor=True).end_date
                addition1_pref_approve_start = ApprovalDate.objects.get(site=data['site'], preference_order=-1,
                                                                        for_instructor=True).start_date
                addition1_pref_approve_end = ApprovalDate.objects.get(site=data['site'], preference_order=-1,
                                                                      for_instructor=True).end_date
                ...
                for course in courses:
                    trainess[course] = {}
                    if (now_for_approve < ApprovalDate.objects.get(site=data['site'], preference_order=-1,
                ...
                if request.POST:
                    logger.info("kursiyer onay islemi basladi", extra=d)
                    logger.info(request.POST, extra=d)
                    for course in courses:
                        try:
                            if (
                                    now_for_approve > first_pref_approve_start and now_for_approve < third_pref_approve_end):
                                allprefs = []
                                ...
                                data["course"] = course
                                send_email("messages/inform_trainers_about_changes_subject.txt",
                                           "messages/inform_trainers_about_changes.html",
                                           "messages/inform_trainers_about_changes.txt",
                                           data,
                                           EMAIL_FROM_ADDRESS,
...
def statistic(request):
    ...
    try:
        ...
        logger.debug(statistic_by_gender, extra=d)
            ...
            for tcr in trainess_course_records:
                try:
                    ...
                    if ((now_for_approve > check_start and now_for_approve < check_end) or (
                        ...
                        send_email("messages/notice_for_canceled_courses_subject.html",
                                   "messages/notice_for_canceled_courses.html",
                                   "messages/notice_for_canceled_courses.text",
                                   context,
                                   EMAIL_FROM_ADDRESS,
...
def approve_course_preference(request):
    ...
    if request.POST:
        try:
            logger.debug(request.POST.get("courseRecordId"), extra=d)
            ...
            if (now_for_approve > pref_approve_start) and (now_for_approve < pref_approve_end):
                ...
                status = "0"
                logger.debug("kursu onayladi " + trainess_course_record.course.name, extra=d)
            else:
    ...
    try:
        ...
        if now_for_approve > first_pref_approve_start and now_for_approve < third_pref_approve_end:
            ...
            if len(trainess_course_record) == 1:
                ...
                if len(trainess_course_record) > 1:
                    logger.debug("kursiyerin teyit edilen 1'den fazla kursu var!!", extra=d)
                ...
                for course_pre in course_prefs:
                    try:
                        ...
                        if course_object.application_is_open:
                            ...
                            course_record.save()
                            logger.debug("ek tercih kaydedildi " + str(course_pre['value']), extra=d)
                        else:
                            message = "Kurs basvurulara kapali"
                            logger.error(message + " " + str(course_pre['value']), extra=d)
                    except Exception as e:
                        logger.error(e.message, extra=d)
                        message = "Tercihleriniz kaydedilirken hata oluştu"
                        return HttpResponse(json.dumps({'status': '-1', 'message': message}),
